...
if (productGrid && Array.isArray(PRODUCTS)) {
  ...
  if (categoryControls) {
    categoryControls.innerHTML = `<label for="shop-category-select">عرض العطور</label><select id="shop-category-select" class="category-select"><option value="all">كل العطور (${PRODUCTS.length})</option>${SHOP_CATEGORIES.map((category) => `<option value="${category.id}">${category.title} (${category.products.length})</option>`).join('')}</select>`;
  }
  ...
  const categorySelect = categoryControls?.querySelector('.category-select');
